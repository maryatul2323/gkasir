...
const Detail = ({ navigation, route }) => {
  ...
  const getDetail = async (res) => {
    try {
      const response = await Api.getDetail(res,id_checkout)
  ...
  const getProduct = async (res) => {
    try {
      const response = await Api.getDetailCO(res,id_checkout)
  ...
  const ItemProduct = (item) => (
    <View style={styles.card} key={item.item.id}>
      <View style={{ flexDirection: 'row',     width : '100%', }}>
        ...
        <View>
          ...
          <Text style={{
            ...
            color: '#09051C',
            marginLeft: 12
          }}>Jumlah pemesanan : {item.item.jumlah_pemesanan}</Text>
          <Text style={{
            fontSize: 15,
            fontWeight: 'bold',
            color: '#FEAD1D',
